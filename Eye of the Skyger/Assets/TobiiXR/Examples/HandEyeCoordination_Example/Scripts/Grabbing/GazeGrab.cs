    ...
    private void UpdateObjectState()
    {
        ...
        if (_currentGrabState == GrabState.Grabbing)
        {
            ...
            if (ControllerManager.Instance.GetButtonPress(TriggerButton))
            {
                ...
                if (Vector3.Distance(_grabbedObjectRigidBody.position, ControllerManager.Instance.Position) < ObjectSnapDistance)
                {
        ...
        if (_currentGrabState == GrabState.Grabbed)
        {
            if (ControllerManager.Instance.GetButtonPress(TriggerButton))
            {
                ...
                _grabbedObjectRigidBody.rotation = (ControllerManager.Instance.Rotation * Quaternion.Inverse(_startControllerRotation)) * _startObjectRotation;
                _grabbedObjectRigidBody.position = ControllerManager.Instance.Position;
            }
            else
            {
                _grabbedObjectRigidBody.angularVelocity = ControllerManager.Instance.AngularVelocity * ObjectVelocityMultiplier;
