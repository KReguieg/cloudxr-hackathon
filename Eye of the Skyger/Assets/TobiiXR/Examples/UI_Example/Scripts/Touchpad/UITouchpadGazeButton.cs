...
public class UITouchpadGazeButton : MonoBehaviour, IGazeFocusable
{
    ...
    private void Update()
    {
        ...
        if (_currentButtonState == ButtonState.Focused && ControllerManager.Instance.GetButtonPressDown(TouchpadButton))
        {
        ...
        else if (_currentButtonState == ButtonState.PressedDown && ControllerManager.Instance.GetButtonPressUp(TouchpadButton))
        {
            // Invoke click event.
            if (OnButtonClicked != null)
            {
