...
public class UITriggerGazeSlider : MonoBehaviour, IGazeFocusable
{
    ...
    [SerializeField, Tooltip("The minimum value of the slider. A big difference between the min and max value creates a continuous slider.")]
    private int _minValue = 0;
    [SerializeField, Tooltip("The maximum value of the slider. A big difference between the min and max value creates a continuous slider.")]
    ...
    {
        return transform.InverseTransformVector(ControllerManager.Instance.Velocity) * _xScaleLossy * Time.deltaTime;
    }
    ...
    private bool SliderValueOutsideBounds()
    {
        if (_maxValue <= _minValue)
        {
            ...
            {
                Debug.LogErrorFormat("{0}'s maximum value ({1}) has to be bigger that the minimum value ({2})", gameObject.name, _maxValue, _minValue);
